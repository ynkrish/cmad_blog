...
(function($) {
	...
	app.controller('AppHomeController', function($http, $log, $scope,
        ...
        //Krish - hack to send the Basic Auth header -not secure
        //What is the best way to secure these credentials ?? 
        //$http.defaults.headers.common['Authorization'] = 'Basic ' + window.btoa('restServicesUser' + ':' + 'Cisco_123');
        //Krish - end of code change
		...
		$http.get('/Services/rest/user?signedIn=true').success(
				function(data, status, headers, config) {
                   ...
                   /* ws = $websocket.$new('ws://'+$location.host()+':'+$location.port()+'/Services/chat', ['binary', 'base64']); // instance of ngWebsocket, handled by $websocket service
					...
					$scope.loading = false;
					$scope.error = status;
                    //Krish - added for redirect to login page in this case
